...
import { ZONES, ITEMS, BADGES, ENCHANTABLE_ATTRIBUTES } from '../../../constants/index';
...
export const collectPay = (playerState: PlayerState, homunculusId: number): EngineResult<{}> => {
    ...
    if (baseEarnings <= 0) {
        return { success: false, message: "Not enough time has passed to collect pay." };
    }
    ...
    const finalEarnings = Math.floor(baseEarnings * wageMultiplier * enchantmentUsdMultiplier);
    ...
    if (finalEarnings > 10000 && !newPlayerState.unlockedBadgeIds.includes('misc_stonks')) {
    ...
    const bonusFromEnchants = enchantmentUsdMultiplier > 1 ? (enchantmentUsdMultiplier - 1) : 0;
    const totalBonus = bonusFromGear + bonusFromEnchants;
    const bonusText = totalBonus > 0 ? ` (incl. ${totalBonus.toLocaleString(undefined, {style: 'percent'})} bonus)` : '';
