...
export const useConsumableItem = (playerState: PlayerState, itemId: string): EngineResult<{}> => {
    ...
    const inventoryItemIndex = newPlayerState.inventory.findIndex(i => i.itemId === itemId);
    ...
    if (itemData.buffEffect) {
        if (newPlayerState.activeBoosts.length >= 5) {
            return { success: false, message: "Maximum number of active buffs reached (5)." };
        }

        const newBoost: ActiveBoost = {
            type: itemData.buffEffect.type,
            value: itemData.buffEffect.value,
            endTime: clockService.getCurrentTime() + itemData.buffEffect.durationSeconds * 1000,
