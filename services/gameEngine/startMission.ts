...
import type { PlayerState, MissionDurationKey, ActiveMission, Rewards } from '../../types';
...
export const startMission = (playerState: PlayerState, zoneId: string, durationKey: MissionDurationKey, isDevMode: boolean): { success: boolean, message: string, activeMission?: ActiveMission } => {
    ...
    const contextualPower = calculatePlayerPower(playerState, { zoneId, durationKey });
    if (contextualPower < zone.requiredPower) {
        return { success: false, message: `Not enough power. Required: ${zone.requiredPower}. You have ${Math.round(playerState.power)}.` };
    }
