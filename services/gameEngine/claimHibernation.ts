...
export const claimHibernation = (playerState: PlayerState, homunculusId: number): EngineResult<{}> => {
    ...
    if (homunculus.hibernationEndTime === null || clockService.getCurrentTime() < homunculus.hibernationEndTime) {
        return { success: false, message: "Hibernation is not yet complete." };
    }

    const updatedHomunculus = { ...homunculus, isAdult: true, hibernationEndTime: null };
