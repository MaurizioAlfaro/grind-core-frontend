...
import type { PlayerState, SlottedParts, Homunculus, HomunculusTraits } from '../../types';
import { ITEMS, LAB_LEVEL_PERKS, LAB_EQUIPMENT, CORE_COMPONENT_ID } from '../../constants/index';
...
export const createHomunculus = (playerState: PlayerState, slottedParts: SlottedParts): EngineResult<{}> => {
    
    // Validate the recipe
    if (slottedParts.core !== CORE_COMPONENT_ID) {
        return { success: false, message: "A Shapeshifter's Gene must be the core component." };
    }
    ...
    const requiredParts = [slottedParts.core, slottedParts.material1, slottedParts.material2];

    // Check if player has all required parts
    const inventoryMap = new Map(playerState.inventory.map(i => [i.itemId, i.quantity]));
    ...
    const legendarySaveBonus = newPlayerState.purchasedLabEquipmentIds.includes('probability_stabilizer') ? 0.05 : 0;
    ...
    newPlayerState.homunculusCreatedCount = (newPlayerState.homunculusCreatedCount || 0) + 1;
