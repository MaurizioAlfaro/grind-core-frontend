...
const createDurations = (baseXp: number, baseGold: number) => ({
  ...
  MEDIUM: {
    ...
    baseGold: Math.round(baseGold * 10),
  },
  LONG: {
    seconds: 28800,
    baseXp: Math.round(baseXp * 50),
    baseGold: Math.round(baseGold * 50),
  },
});
...
export const zone01: Zone = {
  ...
  lootTable: [
    ...
    { itemId: "cafeteria_hairnet", chance: 0.1 },
  ...
  completionBonus: {
    ...
    apply: (state) => ({
      ...state,
      permanentPowerBonus: state.permanentPowerBonus + 5,
    }),
  },
  jobName: "Dish Washer",
  workerLimit: 1,
