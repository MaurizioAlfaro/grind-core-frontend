...
Object.values(ITEMS).forEach((item) => {
  if (!itemCache.byRarity.has(item.rarity))
    itemCache.byRarity.set(item.rarity, []);
  itemCache.byRarity.get(item.rarity)!.push(item.id);

  if (!itemCache.byCategory.has(item.category))
    itemCache.byCategory.set(item.category, []);
...
const bossBadgeUnlocks = Object.values(BOSSES).reduce((acc, boss) => {
  acc[boss.firstVictoryBadgeId] = (p: PlayerState) =>
    p.defeatedBossIds.includes(boss.id);
  return acc;
...
export const BADGE_UNLOCK_CONDITIONS: {
  ...
  collector_common: createSetChecker(
    itemCache.byRarity.get(ItemRarity.Common) || []
  ),
  collector_rare: createSetChecker(
    itemCache.byRarity.get(ItemRarity.Rare) || []
  ),
  collector_epic: createSetChecker(
    itemCache.byRarity.get(ItemRarity.Epic) || []
  ),
  collector_legendary: createSetChecker(
    itemCache.byRarity.get(ItemRarity.Legendary) || []
  ),
  ...
  forge_5: (p) =>
    Object.values(p.equipmentUpgrades).some((level) => level >= 5),
  forge_10: (p) =>
    Object.values(p.equipmentUpgrades).some((level) => level >= 10),
  forge_15: (p) =>
    Object.values(p.equipmentUpgrades).some((level) => level >= 15),
  full_set_5: (p) => {
    // Check if all equipped items have at least +5 upgrades
    return Object.values(EquipmentSlot).every((slot) => {
      const itemId = p.equipment[slot];
      return itemId && (p.equipmentUpgrades[itemId] || 0) >= 5;
    });
  },
  full_set_10: (p) => {
    // Check if all equipped items have at least +10 upgrades
    return Object.values(EquipmentSlot).every((slot) => {
      const itemId = p.equipment[slot];
      return itemId && (p.equipmentUpgrades[itemId] || 0) >= 10;
    });
  },
  full_set_15: (p) => {
    // Check if all equipped items have at least +15 upgrades
    return Object.values(EquipmentSlot).every((slot) => {
      const itemId = p.equipment[slot];
      return itemId && (p.equipmentUpgrades[itemId] || 0) >= 15;
    });
  },
  ...
  lab_equipment_all: (p) =>
    p.purchasedLabEquipmentIds.length >= Object.keys(LAB_EQUIPMENT).length,
  breeder_legendary: (p) =>
    p.homunculi.some((h) => h.rarity === ItemRarity.Legendary),
  trait_max: (p) =>
    p.homunculi.some((h) =>
      Object.values(h.traits).some((t) => t >= MAX_TRAIT_LEVEL)
    ),
  ...
  marketplace_purchase: (p) =>
    p.inventory.some(
      (i) => ITEMS[i.itemId]?.category === ItemCategory.HomunculusGear
    ),
  ...
  misc_the_collector: (p) =>
    ALL_REPTILIAN_PARTS.every((partId) =>
      p.inventory.some((i) => i.itemId === partId)
    ),
  ...
  misc_nudist_colony: (p) =>
    p.homunculi.filter(
      (h) => h.isAdult && Object.keys(h.equipment || {}).length === 0
    ).length >= 5,
  misc_the_gambler: (p) => (p.consecutiveCommonCacheOpens || 0) >= 10,
  misc_forgot_to_clock_out: (p) =>
    p.homunculi.some(
      (h) => h.work && Date.now() - h.work.startTime > 7 * 24 * 60 * 60 * 1000
    ),
  ...
  secret_commoner: (p) => {
    ...
    return slots.every((slot) => {
      const itemId = p.equipment[slot];
      return itemId && ITEMS[itemId]?.rarity === ItemRarity.Common;
    });
  },
  secret_rare: (p) => {
    const slots = Object.values(EquipmentSlot) as EquipmentSlot[];
    ...
    return slots.every((slot) => {
      const itemId = p.equipment[slot];
      return itemId && ITEMS[itemId]?.rarity === ItemRarity.Rare;
    });
  },
  secret_epic: (p) => {
    const slots = Object.values(EquipmentSlot) as EquipmentSlot[];
    ...
    return slots.every((slot) => {
      const itemId = p.equipment[slot];
      return itemId && ITEMS[itemId]?.rarity === ItemRarity.Epic;
    });
  },
  secret_legend: (p) => {
    const slots = Object.values(EquipmentSlot) as EquipmentSlot[];
    ...
    return slots.every((slot) => {
      const itemId = p.equipment[slot];
      return itemId && ITEMS[itemId]?.rarity === ItemRarity.Legendary;
    });
  },
