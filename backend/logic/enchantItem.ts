...
const rollForEnchantment = (
  multiplier: number
): { attributeId: string; tier: number } => {
  ...
  const attributeId =
    possibleAttributes[Math.floor(Math.random() * possibleAttributes.length)];

  return { attributeId, tier };
};

const baseEnchantLogic = (
...
export const enchantItem = (
  ...
  if (logicResult.error || !logicResult.newPlayerState) {
    return {
      success: false,
      message: logicResult.error || "An unknown error occurred.",
    };
  }
  ...
  const currentEnchantments =
    newPlayerState.equipmentEnchantments?.[slot] || [];

  if (currentEnchantments.length >= maxSlots) {
    return {
      success: false,
      message: "No empty enchantment slots on this item.",
    };
  }

  const newEnchantment = rollForEnchantment(costMultiplier);

  const newEquipmentEnchantments = {
    ...(newPlayerState.equipmentEnchantments || {}),
  };
...
export const rerollEnchantment = (
  playerState: PlayerState,
  slot: EquipmentSlot,
