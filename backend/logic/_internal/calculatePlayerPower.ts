...
import {
  ...
  FORGE_CONFIG,
  FORGE_ATTRIBUTES,
  POWER_PER_TRAIT_POINT,
  HOMUNCULUS_RARITY_POWER_MULTIPLIER,
  BADGES,
  ENCHANTABLE_ATTRIBUTES,
} from "../../../constants/index";

export const calculatePlayerPower = (
  ...
  if (playerState.equipmentEnchantments) {
    for (const slot in playerState.equipmentEnchantments) {
      ...
      for (const enchant of enchantments) {
        ...
        switch (attr.type) {
          case "ADD_POWER":
            flatPowerFromEnchants += value;
            break;
          case "MULTIPLY_EQUIPMENT_POWER":
            equipmentPowerMultiplier *= value;
            break;
          case "ADD_POWER_PER_LEVEL":
            powerPerLevelBonus += value;
            break;
          case "POWER_VS_BOSS":
            if (context.bossId) bossPowerMultiplier *= value;
            break;
          case "NEMESIS_PROTOCOL":
            if (context.bossId) {
              const defeatCount =
                playerState.bossDefeatCounts?.[context.bossId] || 0;
  ...
  const powerFromLevel =
    playerState.level * (POWER_PER_LEVEL + powerPerLevelBonus);
  ...
  if (playerState.homunculi) {
    playerState.homunculi.forEach((h) => {
      if (h.isAdult) {
        const totalTraits = Object.values(h.traits).reduce(
          (sum, val) => sum + val,
          0
        );
        const rarityMultiplier = HOMUNCULUS_RARITY_POWER_MULTIPLIER[h.rarity];
        powerFromHomunculi +=
          totalTraits * POWER_PER_TRAIT_POINT * rarityMultiplier;
      }
      if (h.equipment) {
  ...
  const permanentPerks = Array.from(playerState.unlockedPermanentPerks)
    .map((id) => FORGE_ATTRIBUTES[id])
    .filter(Boolean);
  const permanentPowerFromPerks = permanentPerks.reduce(
    (sum, perk) =>
      perk.effect.type === "ADD_PERMANENT_POWER"
        ? sum + perk.effect.value
        : sum,
    0
  );
  ...
  playerState.unlockedBadgeIds.forEach((badgeId) => {
    ...
    if (badge?.bonus) {
      if (badge.bonus.type === "ADD_PERMANENT_POWER")
        permanentPowerFromBadges += badge.bonus.value;
      else if (badge.bonus.type === "MULTIPLY_POWER")
        powerMultiplierFromBadges *= badge.bonus.value;
    }
  });

  // 6. Sum up all base power sources
  let totalPower =
    basePowerFromEquipment +
    powerFromLevel +
  ...
  if (typeof playerState.activeBoosts === "string") {
    console.log(
      "🔍 [calculatePlayerPower] ERROR: activeBoosts became a string during power calculation!"
    );
    console.log(
      "🔍 [calculatePlayerPower] String content:",
