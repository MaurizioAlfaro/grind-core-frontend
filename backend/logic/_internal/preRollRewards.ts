...
export const preRollRewards = (
  ...
  activeAttributes.forEach((attr) => {
    ...
    if (attr.effect.type === "MULTIPLY_LOOT_CHANCE")
      lootMultiplier *= attr.effect.value;
  });
      ...
      if (enchantments) {
        enchantments.forEach((enchant) => {
          ...
          if (attr) {
            ...
            if (attr.type === "MULTIPLY_REPTILIAN_PART_CHANCE")
              reptilianPartMultiplier *= value;
          }
        });
      }
    }
  ...
  zone.lootTable.forEach((loot) => {
    ...
    const finalChance = Math.min(
      currentChance * missionLengthMultiplier * lootMultiplier,
      1
    );
  ...
  Object.values(FOOD_ITEMS).forEach((food) => {
    ...
    const finalChance = Math.min(
      foodChance * missionLengthMultiplier * lootMultiplier,
      1
    );
    if (Math.random() < finalChance) {
  ...
  if (zone.exclusiveLoot) {
    zone.exclusiveLoot.forEach((loot) => {
      if (loot.duration === durationKey && Math.random() < 0.1) {
        // 10% chance for exclusive
        foundItemIds.add(loot.itemId);
      }
    });
  }
