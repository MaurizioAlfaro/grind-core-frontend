...
export const calculateBossWinChance = (
  playerPower: number,
  bossPower: number
): number => {
  ...
  return Math.min(successChance, 0.9);
};
...
export const fightBoss = (
...
): EngineResult<{ rewards?: Rewards; outcome: "win" | "loss" }> => {
  console.log(
    "🔍 [fightBoss] Input playerState.activeBoosts:",
    JSON.stringify(playerState.activeBoosts, null, 2)
  );
  console.log(
    "🔍 [fightBoss] Input playerState.activeBoosts type:",
    typeof playerState.activeBoosts
  );
  console.log(
    "🔍 [fightBoss] Input playerState.activeBoosts isArray:",
  ...
  if (now < cooldownEndTime && !isDevMode) {
    return {
      success: false,
      message: "Boss fights are on a global cooldown.",
      outcome: "loss",
    };
  }
  ...
  if (!isDevMode) {
    newPlayerState.globalBossCooldownEndTime =
      now + BOSS_COOLDOWN_HOURS * 60 * 60 * 1000;
  }

  // Badge logic for "Not a Bitch"
  if (
    successChance < 0.05 &&
    !newPlayerState.unlockedBadgeIds.includes("misc_not_a_bitch")
  ) {
  ...
  if (!didWin) {
    ...
    return {
      success: true,
      message: `You were defeated by ${boss.name}!`,
  ...
  if (!newPlayerState.unlockedBadgeIds.includes("misc_frugal_fighter")) {
    ...
    const equippedItems = slots
      .map((slot) => playerState.equipment[slot])
      .filter(Boolean);
    if (
      equippedItems.length > 0 &&
      equippedItems.every(
        (itemId) => ITEMS[itemId!]?.rarity === ItemRarity.Common
      )
    ) {
  ...
  const baseRewardsData = zone?.missionDurations["LONG"] || {
    baseXp: 1000,
    baseGold: 500,
  };
  ...
  activeAttributes.forEach((attr) => {
    ...
    if (attr.effect.type === "MULTIPLY_GOLD")
      goldMultiplier *= attr.effect.value;
  });
  ...
  console.log(
    "🔍 [fightBoss] Calculated multipliers from activeBoosts - xp:",
    xpMultiplier,
    "gold:",
    goldMultiplier,
    "loot:",
  ...
  const bossRewards: Rewards = {
    xp: Math.round(
      baseRewardsData.baseXp * boss.rewardMultiplier.xp * xpMultiplier
    ),
    gold: Math.round(
      baseRewardsData.baseGold * boss.rewardMultiplier.gold * goldMultiplier
    ),
    dollars: 0,
    items: [],
  };
  ...
  console.log(
    "🔍 [fightBoss] Final result.newPlayerState.activeBoosts:",
    JSON.stringify(badgeCheckResult.newPlayerState.activeBoosts, null, 2)
  );
  console.log(
    "🔍 [fightBoss] Final result.newPlayerState.activeBoosts type:",
    typeof badgeCheckResult.newPlayerState.activeBoosts
  );
  console.log(
    "🔍 [fightBoss] Final result.newPlayerState.activeBoosts isArray:",
