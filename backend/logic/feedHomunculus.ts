...
export const feedHomunculus = (
  ...
  if (homunculus.isAdult || homunculus.hibernationEndTime === null) {
    return {
      success: false,
      message: "This creature can only be fed while hibernating.",
    };
  }
  ...
  updatedHomunculus.traits[foodItemData.trait] = Math.min(
    potentialNewValue,
    MAX_TRAIT_LEVEL
  );
  ...
  if (
    updatedHomunculus.traits[foodItemData.trait] >= MAX_TRAIT_LEVEL &&
    !newPlayerState.unlockedBadgeIds.includes("misc_one_trick_pony")
  ) {
    const otherTraits = Object.entries(updatedHomunculus.traits).filter(
      ([t]) => t !== foodItemData.trait
    );
