...
export const upgradeItem = (
  ...
  const rarityMultiplier =
    RARITY_COST_MULTIPLIER[item.rarity as ItemRarity] || 1;
  const baseCost = upgradeInfo.gold * rarityMultiplier;

  const cost = isSafe
    ? baseCost * FORGE_CONFIG.safeUpgradeCostMultiplier
    : baseCost;
  ...
  if (roll < upgradeInfo.success) {
    ...
    if (upgradeInfo.success >= 0.9) {
      newPlayerState.highChanceForgeFails =
        (newPlayerState.highChanceForgeFails || 0) + 1;
    }
    ...
    if (!isSafe) {
      ...
      if (newPlayerState.equipmentEnchantments) {
        for (const s in newPlayerState.equipmentEnchantments) {
          const enchantments =
            newPlayerState.equipmentEnchantments[s as EquipmentSlot];
          if (enchantments) {
    ...
    if (
      isSafe ||
      savedBySafeguard ||
      roll < upgradeInfo.success + upgradeInfo.stay
    ) {
      outcome = "stay";
      message = savedBySafeguard
        ? `Upgrade failed, but your Safeguard prevented a downgrade!`
