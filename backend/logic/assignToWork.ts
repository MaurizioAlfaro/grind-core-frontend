...
export const assignToWork = (
  ...
  // Worker limit check
  const currentWorkers = playerState.homunculi.filter(
    (h) => h.work?.zoneId === zoneId
  ).length;
  ...
  for (const trait in zone.workRequirements) {
    ...
    if (homunculusLevel < requiredLevel) {
      return {
        success: false,
        message: `Not qualified. Requires ${trait} level ${requiredLevel}.`,
      };
    }
  }
  ...
  const updatedHomunculus = {
    ...homunculus,
    work: { zoneId, startTime: clockService.getCurrentTime() },
  };
