...
import {
  upgradeItem as upgradeItemLogic,
  enchantItem as enchantItemLogic,
  rerollEnchantment as rerollEnchantmentLogic,
} from "../logic";

export const upgradeItem = asyncHandler(async (req: any, res: any) => {
  const { itemId, isSafe } = req.body;
...
export const rerollEnchantment = asyncHandler(async (req: any, res: any) => {
  ...
  const result = rerollEnchantmentLogic(
    playerDoc.toObject(),
    slot,
    enchantmentIndex,
    costMultiplier
