...
const EquipmentSlotDisplay: React.FC<{
  ...
  return (
    <div
      className={`relative flex flex-col items-center justify-center w-20 h-24 p-2 bg-gray-800 rounded-lg border-2 ${
    ...
    >
      ...
      {item ? (
        <>
          ...
          />
          <p className={`text-xs font-bold ${RarityTextMap[item.rarity]}`}>
            {item.name}
          </p>
          {upgradeLevel > 0 && (
            <p className="text-xs font-bold text-yellow-400">+{upgradeLevel}</p>
          )}
          <button
...
const CollectionItem: React.FC<{
  ...
  const baseClasses =
    "relative flex flex-col items-center justify-center p-2 h-24 w-20 bg-gray-800 rounded-lg border-2 transition-all";
  const borderClass = discovered
    ? isEquipped
      ? "border-yellow-400"
      : RarityBorderMap[item.rarity]
    : "border-gray-700";
  const buttonClasses = canInteract
  ...
  return (
    <div className="relative">
      ...
      >
        {discovered ? (
          ...
          <>
            <ItemIcon
              item={item}
              className="text-3xl filter brightness-0 invert opacity-50"
              imgClassName="w-10 h-10 object-contain filter brightness-0 invert opacity-50"
            />
      ...
      <button
        ...
        className="absolute top-0 right-0 p-1 text-gray-500 hover:text-cyan-300 z-10"
...
const ZoneCollection: React.FC<
  Pick<CollectionProps, "player" | "onEquip" | "onShowItemInfo">
> = ({ player, onEquip, onShowItemInfo }) => {
  const visibleZones = ZONES.filter((zone) =>
    player.unlockedZoneIds.includes(zone.id)
  );
  ...
  return (
    <div className="space-y-6">
      {visibleZones.map((zone) => {
        ...
        const items = Array.from(zoneItems)
          .map((id) => ITEMS[id])
          .filter(Boolean);
        const discoveredCount = items.filter((item) =>
          player.discoveredItemIds.includes(item.id)
        ).length;
          ...
          >
            <div className="flex justify-between items-baseline mb-2">
              ...
              <span className="text-sm text-gray-400">
                {discoveredCount} / {items.length} Found
              </span>
            </div>
            ...
            >
              {isCompleted ? "★" : "☆"} Completion Bonus:{" "}
              {zone.completionBonus.description}
            </div>
          </div>
        );
      })}
...
> = ({ player, onEquip, onShowItemInfo, category, title }) => {
  const equippedItemIds = new Set(Object.values(player.equipment));
  const itemsInCategory = Object.values(ITEMS).filter(
    (item) => item.category === category
  );
  const discoveredCount = itemsInCategory.filter((item) =>
    player.discoveredItemIds.includes(item.id)
  ).length;
  ...
  return (
    <div className="p-4 bg-gray-800/50 rounded-xl border border-gray-700">
      <div className="flex justify-between items-baseline mb-2">
        ...
        <span className="text-sm text-gray-400">
          {discoveredCount} / {itemsInCategory.length} Found
        </span>
      </div>
      <div className="flex flex-wrap gap-2 p-2 bg-gray-900/50 rounded-lg">
