...
export const ForgeSlot: React.FC<ForgeSlotProps> = ({ slot, itemId, upgradeLevel, isSelected, onSelect, upgradeResult }) => {
  const item = itemId ? ITEMS[itemId] : null;

  const getResultClasses = () => {
    if (!upgradeResult || upgradeResult.slot !== slot) return '';
    ...
    >
        ...
        {item ? (
            <>
                ...
                {upgradeLevel > 0 && <p className="text-sm font-bold text-yellow-400">+{upgradeLevel}</p>}
            </>
        ) : (
