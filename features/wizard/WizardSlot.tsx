...
export const WizardSlot: React.FC<WizardSlotProps> = ({
  slot,
  player,
  isSelected,
  onSelect,
  ...
  const upgradeLevel = itemId ? player.equipmentUpgrades[itemId] || 0 : 0;
  const enchantments = player.equipmentEnchantments?.[slot] || [];
  const maxEnchantments = item
              ...
              {Array.from({ length: maxEnchantments }).map((_, i) => (
                <div
                  ...
                  className={`w-2 h-2 rounded-full ${
                    i < enchantments.length ? "bg-cyan-400" : "bg-gray-600"
                  }`}
                ></div>
              ))}
            </div>
