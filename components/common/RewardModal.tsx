        ...
        <div className="p-6 text-center">
          ...
          <div className="space-y-4 mb-8">
            ...
            {rewards.dollars > 0 && (
                ...
                <span className="text-xl font-bold text-green-400">+${rewards.dollars.toLocaleString()}</span>
                </div>
            )}
          </div>
                ...
                <h3 className="text-xl font-orbitron text-gray-300 mb-4">Items Found</h3>
                 <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 p-2 bg-gray-900/50 rounded-lg">
                    {rewards.items.map((item, index) => {
                        ...
                        return (
                            <div key={`${item.itemId}-${index}`} className={`flex flex-col items-center justify-center p-1 bg-gray-800 rounded-lg border-2 ${RarityColorMap[itemData.rarity]} h-20`}>
                                <ItemIcon item={itemData} className="text-2xl" imgClassName="w-8 h-8 object-contain" />
                                <span className="text-[11px] text-center leading-tight mt-1">{itemData.name}</span>
                                {item.quantity > 1 && <span className="text-[11px] text-cyan-400">x{item.quantity}</span>}
                            </div>
                        )
                    })}
                </div>
          ...
          <StyledButton id="reward-modal-close-btn" onClick={onClose} className="mt-8 w-full">
            Awesome!
          </StyledButton>
        </div>
      </div>
