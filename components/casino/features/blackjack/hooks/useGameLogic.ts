...
export const useGameLogic = () => {
  ...
  const placeBet = (amount: number) => performAction(blackjackApi.placeBet(amount));
  ...
  const decideInsurance = (accepted: boolean) => performAction(blackjackApi.decideInsurance(accepted));
  const decideEvenMoney = (accepted: boolean) => performAction(blackjackApi.decideEvenMoney(accepted));
  ...
  const handleSaveSettings = (newSettings: GameSettings) => performAction(blackjackApi.updateSettings(newSettings));
  
  return {
    settings, deck, dealerHand, gameState, chips, playerHands, bets, 
    activeHandIndex, gameResults, mainBet, lastBet, netRoundWinnings,
    tableMessage,
    log,
    isLoading,
    actions: {
      placeBet, clearBet, repeatBet, maxBet,
      deal, decideInsurance, decideEvenMoney, nextRound,
