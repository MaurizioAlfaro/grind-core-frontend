...
const Controls: React.FC<{
  ...
  isLoading: boolean;
  totalBet: number;
}> = ({ game, perms, onDeal, isLoading, totalBet }) => {
  ...
  switch (gameState) {
    case GameState.BETTING:
      return (
        <BettingControls
          ...
          onClear={actions.clearBet}
          onDeal={onDeal}
          onRepeat={actions.repeatBet}
          onMax={actions.maxBet}
          mainBet={mainBet}
          lastBet={lastBet}
          playerChips={chips}
          settings={settings}
    ...
    case GameState.PLAYER_TURN:
      return (
        <GameControls
          ...
          canDouble={perms.canDouble}
          canSplit={perms.canSplit}
          canSurrender={perms.canSurrender}
          isLoading={isLoading}
        />
      );
    case GameState.INSURANCE_BETTING:
      return (
        <InsurancePrompt
          onInsurance={actions.decideInsurance}
          insuranceCost={totalBet / 2}
          canAfford={chips >= totalBet / 2}
          isLoading={isLoading}
        />
      );
    case GameState.EVEN_MONEY_CHOICE:
      return (
        <EvenMoneyPrompt
          onEvenMoney={actions.decideEvenMoney}
          payout={totalBet}
          isLoading={isLoading}
        />
      );
    case GameState.ROUND_OVER:
      return (
        <button
          ...
          disabled={isLoading}
          className="px-8 py-3 sm:px-10 sm:py-4 bg-yellow-500 text-gray-900 font-bold text-lg sm:text-xl rounded-lg shadow-lg hover:scale-105 disabled:opacity-50 disabled:cursor-wait"
        >
          Next Round
        </button>
      );
...
const BlackjackGame: React.FC = () => {
  ...
  const totalBet = useMemo(
    ...
    [gameState, mainBet, bets]
  );

  useEffect(() => {
    if (
      gameState === GameState.INSURANCE_BETTING &&
      prevGameState !== GameState.INSURANCE_BETTING
    ) {
      setIsPeeking(true);
