...
const BaccaratGame: React.FC = () => {
  ...
  useEffect(() => {
    if (
      ...
      prevGameState === BaccaratGameState.ROUND_OVER
    ) {
  ...
  useEffect(() => {
    if (
      gameState === BaccaratGameState.ROUND_OVER &&
      prevGameState !== BaccaratGameState.ROUND_OVER
    ) {
      setIsRevealComplete(false);
      ...
      steps.push({
        fn: () => {
          setRevealed({
            player: Array(3).fill(false),
            banker: Array(3).fill(false),
          });
      ...
      const createRevealStep =
        (hand: "player" | "banker", index: number) => () => {
          setRevealed((prev) => {
      ...
      if (playerNeedsThird) {
        steps.push({
          fn: () => setDisplayedPlayerHand(finalPlayerHand),
          delay: 200,
        });
      ...
      if (bankerNeedsThird) {
        steps.push({
          fn: () => setDisplayedBankerHand(finalBankerHand),
          delay: 200,
        });
          ...
          <div className="flex flex-col items-center space-y-2 p-1">
            ...
            <div className="flex space-x-2 sm:space-x-4 mt-1">
              <button
                ...
                className="px-4 py-2 sm:px-8 sm:py-3 text-sm sm:text-base bg-green-600 text-white font-bold rounded-lg shadow-lg hover:scale-105 disabled:bg-gray-500"
              >
                Deal
              </button>
              <button
                ...
                disabled={isLoading || totalBet === 0}
                className="px-4 py-2 sm:px-8 sm:py-3 text-sm sm:text-base bg-red-600 text-white font-bold rounded-lg shadow-lg hover:scale-105 disabled:bg-gray-500"
              >
                Clear
              </button>
              <button
                ...
                className="px-4 py-2 sm:px-8 sm:py-3 text-sm sm:text-base bg-blue-600 text-white font-bold rounded-lg shadow-lg hover:scale-105 disabled:bg-gray-500"
              >
                Repeat
              </button>
            </div>
        ...
        if (isRevealComplete) {
          return (
            <button
              ...
              disabled={isLoading}
              className="px-8 py-3 sm:px-10 sm:py-4 bg-yellow-500 text-gray-900 font-bold text-lg sm:text-xl rounded-lg shadow-lg hover:scale-105 disabled:bg-gray-500"
            >
              Next Round
            </button>
          );
