...
const App: React.FC = () => {
    const { 
        ...
        isDevMode, rewardsInfoModalZoneId, isCancelConfirmModalOpen, itemInfoModalId, 
        cacheContentsModalId, forgeUpgradeResult, loreModalZoneId, unlockedBadgesModal, 
        viewingProfile, unlockZoneModalId, globalBossTimer, feedingHomunculus, 
        workplaceHomunculus, equippingHomunculus, resetConfirmation, isInitialBoostModalOpen, 
        isConnectWalletModalOpen, viewingBuffInfo, tutorialConfig, activeView, selectedForgeSlot, actions 
    } = useGameLoop();
    
    // States for new modals
    const [bossLoreModalId, setBossLoreModalId] = useState<string | null>(null);
    const [bossRewardsModalId, setBossRewardsModalId] = useState<string | null>(null);

    const openBossLoreModal = (bossId: string) => setBossLoreModalId(bossId);
    const closeBossLoreModal = () => setBossLoreModalId(null);
    const openBossRewardsModal = (bossId: string) => setBossRewardsModalId(bossId);
    const closeBossRewardsModal = () => setBossRewardsModalId(null);

    if (isLoading || !isInitialized || !gameState) {
        return <div className="min-h-screen flex items-center justify-center text-cyan-400 font-orbitron text-2xl">Loading Game Data...</div>;
    }

    const isLabUnlocked = gameState.player.level >= 20;
    const fifthZoneId = ZONES[4].id; // post_office
    const isBossesUnlocked = gameState.player.unlockedZoneIds.includes(fifthZoneId);
    ...
    const ContextualNavButton: React.FC<{
        ...
        const viewDetails: { [key in AppView]?: { icon: React.ReactNode, label: string } } = {
        ...
        return (
            <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 z-30">
                ...
                >
                    <div className={`transition-transform ${isLocked ? 'text-gray-500' : 'text-cyan-400 group-hover:scale-110'}`}>{action.icon}</div>
                    <span className={`text-sm font-bold ${isLocked ? 'text-gray-500' : ''}`}>{isLocked ? lockMessage : action.label}</span>
                </button>
            </div>
        );
    };

    const LockedView: React.FC<{ icon: React.ReactNode, title: string, message: string }> = ({ icon, title, message }) => (
    ...
    const renderContent = () => {
        switch(activeView) {
            ...
            case 'bosses':
                 if (!isBossesUnlocked) {
                    return <LockedView icon={<BossIcon />} title="Bosses" message={`Unlock the ${ZONES[4].name} zone to face this threat.`} />
                }
                return <BossesView
    ...
    return (
        <>
            ...
            <div className="max-w-xl mx-auto flex flex-col min-h-screen">
                {isInitialBoostModalOpen && <InitialBoostModal onClose={actions.closeInitialBoostModal} />}
                {isConnectWalletModalOpen && <ConnectWalletModal onConnect={actions.connectWallet} onClose={actions.closeConnectWalletModal} />}
                {viewingBuffInfo && <BuffInfoModal buff={viewingBuffInfo} onClose={actions.closeBuffInfoModal} />}
                {modalContent && <RewardModal title={modalContent.title} rewards={modalContent.rewards} onClose={actions.closeModal} />}
                {rewardsInfoModalZoneId && <RewardsInfoModal zoneId={rewardsInfoModalZoneId} discoveredItemIds={gameState.player.discoveredItemIds} onClose={actions.closeRewardsInfoModal} onShowItemInfo={actions.openItemInfoModal} />}
                {itemInfoModalId && <ItemInfoModal itemId={itemInfoModalId} onClose={actions.closeItemInfoModal} />}
                {cacheContentsModalId && <CacheContentsModal cacheId={cacheContentsModalId} onClose={actions.closeCacheContentsModal} onShowItemInfo={actions.openItemInfoModal} />}
                {loreModalZoneId && <LoreModal zoneId={loreModalZoneId} onClose={actions.closeLoreModal} />}
                {unlockZoneModalId && <UnlockZoneModal zoneId={unlockZoneModalId} onConfirm={actions.confirmUnlockZone} onCancel={actions.closeUnlockZoneModal} />}
                {unlockedBadgesModal.length > 0 && <BadgeUnlockModal badges={unlockedBadgesModal} onClose={actions.closeBadgeUnlockModal} />}
                {viewingProfile && <ProfileModal profile={viewingProfile} onClose={actions.closeProfileModal} />}
                {feedingHomunculus && <FeedingModal homunculus={feedingHomunculus} player={gameState.player} onFeed={actions.feedHomunculus} onClose={actions.closeFeedingModal} />}
                {workplaceHomunculus && <WorkplaceModal homunculus={workplaceHomunculus} player={gameState.player} onAssign={actions.assignToWork} onClose={actions.closeWorkplaceModal} />}
                {equippingHomunculus && <HomunculusEquipmentModal homunculus={equippingHomunculus} player={gameState.player} onEquip={actions.equipHomunculusItem} onUnequip={actions.unequipHomunculusItem} onClose={actions.closeEquippingModal} onShowItemInfo={actions.openItemInfoModal}/>}
                ...
                {bossLoreModalId && <BossLoreModal bossId={bossLoreModalId} onClose={closeBossLoreModal} />}
                {bossRewardsModalId && <BossRewardsInfoModal bossId={bossRewardsModalId} onClose={closeBossRewardsModal} onShowItemInfo={actions.openItemInfoModal} />}
                ...
                <header className="p-4 pt-12 sticky top-0 bg-gray-900 z-20">
                    <div className="relative bg-gray-900/80 backdrop-blur-sm rounded-xl border border-gray-700 shadow-lg p-4 space-y-4">
                        ...
                        <ActiveBuffs activeBoosts={gameState.player.activeBoosts} onViewBuff={actions.openBuffInfoModal} />
                        <ContextualNavButton activeView={activeView} onNavigate={actions.setActiveView} />
                    </div>
                </header>
                ...
                <main className="flex-grow p-4 space-y-4 pb-40">
                    ...
                    {lastMessage && <div className="fixed bottom-24 left-1/2 -translate-x-1/2 w-full max-w-md p-4 z-30 text-center text-sm text-yellow-300 "><span className="bg-yellow-900/80 backdrop-blur-sm p-3 rounded-md">{lastMessage}</span></div>}
                </main>
